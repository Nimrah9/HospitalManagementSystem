...
{
    public partial class WalkinPatient : Form
    {
        ...
        private void Register_Click(object sender, EventArgs e)
        {
            using (OleDbConnection con = new OleDbConnection())
            {
                OleDbCommand command = new OleDbCommand("select * from walkinpatient where walkinpatientid = '" + textBox1.Text + "'", con);
                ...
                if (reader.HasRows == false)
                {
                    ...
                    OleDbCommand insertCommand = new OleDbCommand("insert into walkinpatient (walkinpatientid, tokennumber,doctorid) values (" + textBox1.Text + ",'" + textBox6.Text + "','" + textBox7.Text +"')", con);
                    try
                    {
                        insertCommand.ExecuteNonQuery();
                        MessageBox.Show("Patient registered successfully");
                        DataTable dt = new DataTable();
                        OleDbDataAdapter da = new OleDbDataAdapter("select * from  walkinpatient", con);
                        da.Fill(dt);
        ...
        private void WalkinPatient_Load(object sender, EventArgs e)
        {
            using (OleDbConnection con = new OleDbConnection())
            {
                con.Open();
