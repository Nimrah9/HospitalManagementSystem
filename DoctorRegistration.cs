...
namespace WindowsFormsApplication1
{
    public partial class DoctorRegistration : Form
    {
        ...
        private void registerdoctor_Click(object sender, EventArgs e)
        {
            using (OleDbConnection con = new OleDbConnection())
            {
                OleDbCommand command = new OleDbCommand("select * from doctor where doctorid = '" + textBox5.Text + "'", con);
                ...
                if (reader.HasRows == false)
                {
                    ...
                    OleDbCommand insertCommand = new OleDbCommand("insert into doctor (doctorid, dtype, dspecialization, doctorfee) values (" + textBox5.Text + ",'" + textBox8.Text + "','" + textBox9.Text +"','" + textBox10.Text + "')", con);
                    try
                    {
                        insertCommand.ExecuteNonQuery();
                        MessageBox.Show("Doctor registered successfully");
                        DataTable dt = new DataTable();
                        OleDbDataAdapter da = new OleDbDataAdapter("select * from  doctor", con);
                        da.Fill(dt);
                ...
                {
                    MessageBox.Show("Doctor is already registered.");
                }
                reader.Close();
            }
        }
