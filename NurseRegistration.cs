...
namespace WindowsFormsApplication1
{
    public partial class NurseRegistration : Form
    {
        ...
        private void getemployeedata_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection con = new OleDbConnection())
            {
                ...
                while (reader1.Read())
                {
                    ...
                    textBox6.Text = Convert.ToString(ejob);
                    etimings = reader1.GetString(6);
                    textBox7.Text = Convert.ToString(ejob);
        ...
        private void registernurse_Click(object sender, EventArgs e)
        {
            using (OleDbConnection con = new OleDbConnection())
            {
                OleDbCommand command = new OleDbCommand("select * from nurse where nurseid = '" + textBox5.Text + "'", con);
                ...
                if (reader.HasRows == false)
                {
                    ...
                    OleDbCommand insertCommand = new OleDbCommand("insert into nurse (nurseid,nursecharges) values (" + textBox5.Text + ",'" + textBox8.Text + "')", con);
                    try
                    {
                        insertCommand.ExecuteNonQuery();
                        MessageBox.Show("Nurse registered successfully");
                        DataTable dt = new DataTable();
                        OleDbDataAdapter da = new OleDbDataAdapter("select * from  nurse", con);
                        da.Fill(dt);
                ...
                {
                    MessageBox.Show("Nurse is already registered.");
                }
                reader.Close();
            }
        }
